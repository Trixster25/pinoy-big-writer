...
function Spelling() {
  ...
  const renderLevelIcon = (completed: boolean, index: number) => {
    ...
    return completed ? (
      <FaStar key={index} className="text-9xl text-green-500" />
    ) : (
      <FaRegStar key={index} className="text-9xl text-green-500" />
    );
  };

  return (
    <div className="min-h-screen flex flex-col gap-8 items-center justify-around bg-white p-4 font-sans spelling">
      {/* Title */}
      <motion.div
        className="flex justify-center bg-green-500 px-6 py-3 rounded-xl"
        ...
        {levels?.map((completed, idx) => {
          ...
          const levelContent = (
            ...
            >
              ...
              {unlocked && (
                <span className="text-center text-3xl text-green-500 font-semibold break-words">
                  {levelsName[idx]}
                </span>
              )}
            </motion.div>
      ...
      <Link to="/games">
        <motion.div
          className="w-24 h-24 bg-green-500 text-white rounded-full flex items-center justify-center cursor-pointer"
