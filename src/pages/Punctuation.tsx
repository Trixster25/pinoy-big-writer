...
import { BiQuestionMark } from "react-icons/bi"; // Import a punctuation icon
...
function Punctuation() {
  ...
  const renderLevelIcon = (completed: boolean, index: number) => {
    ...
    return completed ? (
      <FaStar key={index} className={`${iconSize} text-red-400`} />
    ) : (
      <FaRegStar key={index} className={`${iconSize} text-red-400`} />
    );
  };
  ...
  return (
    ...
    >
      ...
      <div
        ...
        } border-8 rounded-xl rounded-tl-none border-black/50 bg-[#F8D7D7]/25`}
      >
